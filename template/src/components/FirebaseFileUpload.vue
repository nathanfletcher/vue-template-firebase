<template>
    <div>
        ...
        <!-- <button v-click="upload">upload</button> -->
        <p v-if="uploadEnd">{{progressUpload}}%</p>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            uploadMultiple(file) {
                //console.log("starting upload",firebase)
                let vm = this;
                let storageRef = firebase.storage().ref("ketecodeTemplate/" + file.name)
                ...
                let task = storageRef.put(file)
                    /* .then(fileData => {
                        ...
                        this.downloadURL.push[fileData.metadata.downloadURL[0]]
                    }); */
                ...
                task.on('state_changed',
                    function progress(snapshot) {
                        ...
                        //let progPercentage = snapshot.bytesTransferred / snapshot.totalBytes * 100;
                        //console.log("Uploading...", progPercentage + "%")
                    },
                    function error(err) {
